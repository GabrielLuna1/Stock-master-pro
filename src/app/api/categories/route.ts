...
export async function DELETE(request: Request) {
  try {
    ...
    // Busca a categoria para saber o nome exato dela
    ...
    // 🛡️ 2. TRAVA DE INTEGRIDADE (COM REGEX)
    // Usamos RegExp para garantir que "Hardware" bata com "HARDWARE"
    // ^ = começa com, $ = termina com, "i" = ignora maiúsculas
    ...
    if (productsUsingCategory > 0) {
      return NextResponse.json(
        {
          error: `Bloqueado! Existem ${productsUsingCategory} produtos vinculados a esta categoria.`,
        },
        { status: 400 }, // Bad Request
      );
    }

    // 3. Se passou nas checagens (count === 0), exclui
