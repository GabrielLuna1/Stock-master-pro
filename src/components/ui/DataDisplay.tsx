...
// Definimos o que é uma "Coluna" para o nosso componente
interface Column<T> {
  header: string;
  accessorKey: keyof T; // A chave do objeto (ex: "name", "price")
  // Função opcional para formatar o dado (ex: colocar R$ ou cor)
...
interface DataDisplayProps<T> {
  data: T[];
  columns: Column<T>[];
  titleField: keyof T; // Qual campo é o "Título" no Card mobile? (ex: "name")
  subtitleField?: keyof T; // Qual campo é o subtítulo? (ex: "sku")
  onEdit?: (item: T) => void;
...
export function DataDisplay<T extends { _id: string }>({
  ...
  return (
    <>
      {/* 🖥️ VISÃO DESKTOP (Tabela Clássica) - Só aparece em telas md ou maiores */}
      ...
      {/* 📱 VISÃO MOBILE (Cards) - Só aparece em telas pequenas */}
      <div className="md:hidden flex flex-col gap-4">
        {data.map((item) => (
          ...
          >
            ...
            {/* Corpo do Card (Campos Dinâmicos) */}
            <div className="space-y-2 mb-4">
              {columns.map((col, idx) => {
                // Não mostramos o título/subtítulo de novo no corpo pra não repetir
                if (
            ...
            {/* Botões de Ação Grandes (Fáceis de Tocar) */}
