...
import { X, Save, Lock, User, Shield, Loader2 } from "lucide-react";
          ...
          <header className="flex justify-between items-start mb-8">
            <div>
              <h2 className="text-2xl font-black text-gray-900 uppercase tracking-tighter flex items-center gap-2">
                Editar Membro <span className="text-brand-red">Time</span>
              </h2>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">
                Atualizar permissões e dados
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="p-4 rounded-2xl bg-red-50 border border-red-100 space-y-2">
              <label className="text-[10px] font-black text-red-400 uppercase tracking-widest flex items-center gap-1">
                <Lock size={10} /> Resetar Senha (Opcional)
              </label>
              <input
                type="password"
                ...
                className="w-full p-3 bg-white border border-red-100 rounded-xl focus:ring-2 focus:ring-red-200 focus:border-red-300 outline-none font-bold text-gray-800 placeholder:text-gray-300 text-sm"
              />
            </div>
